...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # pos/neg 分开统计 Metrics
    pos_response_length_mean = response_length[pos_rollout_idx].mean().detach().item()
    pos_clip_ratio = torch.mean(torch.eq(response_length[pos_rollout_idx], max_response_length).float()).detach().item()

    neg_response_length_mean = response_length[neg_rollout_idx].mean().detach().item()
    neg_clip_ratio = torch.mean(torch.eq(response_length[neg_rollout_idx], max_response_length).float()).detach().item()

    # pos/neg 分开统计 LogProbs
    ...
    metrics = {
        ...
        **(
        ...
        # pos/neg response length
        "response_length/mean/pos": pos_response_length_mean,
        "response_length/clip_ratio/pos": pos_clip_ratio,
        "response_length/mean/neg": neg_response_length_mean,
        "response_length/clip_ratio/neg": neg_clip_ratio,
