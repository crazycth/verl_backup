...
class SwanLabTableLogger:
    def __init__(self, project_name, experiment_name):
        import swanlab

        self.project_name = project_name
        self.experiment_name = experiment_name
        self.swanlab = swanlab  # 保存引用到实例变量
        swanlab.init(project=project_name, experiment_name=experiment_name)


    def log(self, headers, data, step, key):
        table = self.swanlab.echarts.Table()
        table.add(headers, data)
        self.swanlab.log({key: table}, step=step)